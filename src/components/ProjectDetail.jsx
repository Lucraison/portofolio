    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)', fontFamily: 'var(--sans)' }}>
      ...
      {lightbox !== null && (
        <div
          ...
          style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'zoom-out', padding: '40px' }}
        >
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, display: 'flex', alignItems: 'center', padding: '0 40px', height: '56px', borderBottom: '0.5px solid var(--border)', background: 'var(--bg)', transform: 'translateZ(0)' }}>
        ...
        <span style={{ fontFamily: 'var(--mono)', fontSize: '14px', color: 'var(--accent)', letterSpacing: '0.05em', marginLeft: 'auto' }}>
          NH_
        </span>
      </nav>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '120px 40px 80px' }}>
        ...
        <div style={{ borderTop: '0.5px solid var(--border)', paddingTop: '48px', marginBottom: '64px' }}>
          {project.longDesc.split('\n\n').map((para, i) => (
            <p key={i} style={{ fontSize: '15px', color: 'var(--muted)', lineHeight: 1.9, marginBottom: '20px' }}>
              {para}
            </p>
          ))}
        </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '12px' }}>
              {project.images.map((src, i) => (
                <div
                  ...
                  style={{ border: '0.5px solid var(--border)', overflow: 'hidden', cursor: 'zoom-in', background: 'var(--bg1)' }}
                >
