...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    const { id, name, year, desc, content, tags, status, url, guideUrl, images, blocks, featured } = req.body
    if (!id || !name) return res.status(400).json({ error: 'id and name are required' })
    const existing = await col.findOne({ id })
    if (existing) return res.status(409).json({ error: 'Project with this id already exists' })
    const project = { id, name, year: year || '', desc: desc || '', content: content || '', tags: tags || [], status: status || 'unfinished', url: url || '', guideUrl: guideUrl || '', images: images || [], blocks: blocks || [], featured: featured || false, createdAt: new Date(), updatedAt: new Date() }
